...
export function Layout({
  ...
  const getNotificationColor = (type: string) => {
    switch (type) {
      ...
      default:
        return "bg-[#0070f3] shadow-[0_0_10px_rgba(0,112,243,0.5)]";
    }
  };
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-sans selection:bg-primary/30">
        ...
        <div className="h-full flex items-center justify-between px-6 lg:px-8">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <h1 className="text-lg font-black tracking-tight hidden sm:block uppercase">
                {companyName}
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2 md:gap-4">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild id="notifications-bell">
                <Button
                  variant="ghost"
                  size="icon"
                  className="relative rounded-full hover:bg-white/5 transition-colors group"
                >
                  <Bell
                    className={`h-5 w-5 transition-colors ${unreadCount > 0 ? "text-[#00f2ff] animate-pulse" : "text-slate-400 group-hover:text-white"}`}
                  />
                  {unreadCount > 0 && (
              ...
              <DropdownMenuContent
                align="end"
                className="w-80 rounded-2xl p-0 border-white/10 bg-black/80 backdrop-blur-xl shadow-[0_0_30px_rgba(0,112,243,0.2)] overflow-hidden"
              >
                <div className="px-5 py-4 border-b border-white/10 flex items-center justify-between bg-white/[0.02]">
                  ...
                  {unreadCount > 0 && (
                    <Button
                      ...
                      className="h-7 px-3 text-[9px] font-black uppercase text-slate-400 hover:text-[#00f2ff] hover:bg-white/5 rounded-lg tracking-wider"
                    >
                  ...
                  {notifications.length === 0 ? (
                    ...
                    notifications.map((n) => (
                      ...
                      >
                        ...
                        <div
                          className={`absolute left-0 top-0 bottom-0 w-[2px] transition-all duration-300 ${!n.read ? getNotificationColor(n.type) : "bg-transparent"}`}
                        />

                        <div
                          className={`mt-1 p-2 rounded-xl transition-all duration-300 ${!n.read ? "bg-white/5 shadow-[0_0_10px_rgba(255,255,255,0.05)] border border-white/10" : "bg-transparent opacity-50"}`}
                        >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between gap-2">
                            <p
                              className={`text-[11px] font-black uppercase tracking-tight ${!n.read ? "text-white" : "text-slate-400"}`}
                            >
                              {n.title}
                            </p>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild id="user-profile">
                <Button
                  variant="ghost"
                  className="flex items-center gap-3 px-2 hover:bg-muted transition-colors rounded-xl h-10 border border-border"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-56 rounded-2xl p-1.5 border-border bg-card shadow-xl"
              >
                ...
                <DropdownMenuItem
                  onClick={onLogout}
                  className="text-destructive rounded-xl cursor-pointer font-bold text-xs py-2.5 focus:bg-destructive/10 focus:text-destructive uppercase tracking-widest"
                >
      ...
      <aside
        id="sidebar-nav"
        className={`fixed left-0 top-16 bottom-0 w-64 bg-card border-r border-border z-40 transition-transform lg:translate-x-0 ${sidebarOpen ? "translate-x-0 shadow-2xl" : "-translate-x-full"}`}
      >
          ...
          {navItems.map((item) => {
            ...
            const isAdmin =
              ...
              (typeof user.role === "object" &&
                (user.role as any).name === "Administrador") ||
              user.role === "EMPRESA";
            const isTechnician =
              typeof user.role === "object" &&
              (user.role as any).name === "Técnico";
            if (
              (item.id === "team" ||
                item.id === "roles" ||
                item.id === "areas") &&
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold transition-all group uppercase tracking-tighter ${isActive ? "bg-primary text-primary-foreground shadow-sm shadow-primary/20" : "text-muted-foreground hover:bg-muted hover:text-foreground"}`}
              >
                <Icon
                  className={`h-4 w-4 ${isActive ? "text-primary-foreground" : "text-muted-foreground group-hover:text-foreground transition-colors"}`}
                />
                {item.label}
              </button>
            );
